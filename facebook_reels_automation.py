"""
Facebook Reels Automation - Bilingual English/Finnish Content Generator
IMPROVED VERSION: Better backgrounds, English categories, no repeats, Velocity Finnish branding
"""
...
# Finnish translations for display
CATEGORIES_FINNISH = {
    ...
    "Self Improvement": "Itsekehitys",
    "Gratitude": "Kiitollisuus",
    "Friendship": "Ystävyys",
    "Hope": "Toivo",
    "Creativity": "Luovuus",
    "Inner Peace": "Sisäinen rauha",
    "Confidence": "Itseluottamus",
    "Perseverance": "Sinnikkyys",
    "Inspiration": "Inspiraatio",
    "Positive Life": "Positiivinen elämä",
    "Courage": "Rohkeus",
...
def generate_phrases(category_english: str, num_phrases: int = 5) -> list:
    ...
    category_finnish = CATEGORIES_FINNISH[category_english]
    ...
    for attempt in range(max_attempts):
        try:
            ...
            prompt = f"""Create {num_phrases * 2} unique {category_english} phrases for English speakers learning Finnish.
...
2. Finnish translation (with commas matching the rhythm)
...
IMPORTANT: Create FRESH, UNIQUE phrases that haven't been used before."""
            ...
            payload = {
                "model": AI_MODEL,
                "messages": [
                    {"role": "system", "content": "You are a Finnish teacher. Create short, natural phrases with pauses."},
...
def get_fresh_fallback_phrases(category: str, num_phrases: int) -> list:
    ...
    all_fallbacks = {
        "Motivation": [
            {"english": "Believe in yourself.", "finnish": "Usko itseesi.", "pronunciation": "Oos-ko it-seh-see."},
            {"english": "You are capable of amazing things.", "finnish": "Pystyt uskomattomiin asioihin.", "pronunciation": "Poos-tut oos-ko-mat-to-miin ah-see-oh-in."},
            {"english": "Dream big, start small.", "finnish": "Unelmoi isosti, aloita pienesti.", "pronunciation": "Oo-neh-moi ee-sos-tee, ah-loi-tah pee-eh-nes-tee."},
            {"english": "Your future is created by your actions.", "finnish": "Tulevaisuutesi luovat tekosi.", "pronunciation": "Too-leh-vai-soo-teh-see loo-oh-vat teh-ko-see."},
            {"english": "Never give up on your dreams.", "finnish": "Älä koskaan luovuta unelmistasi.", "pronunciation": "El-ä kos-kaan loo-vo-oo-tah oo-neh-lis-tah-see."},
        ],
        "Love": [
            {"english": "Love yourself first.", "finnish": "Rakasta itseäsi ensin.", "pronunciation": "Rah-kas-tah it-seh-see en-sin."},
            {"english": "Love makes everything possible.", "finnish": "Rakkaus tekee kaiken mahdolliseksi.", "pronunciation": "Rahk-kaus teh-kee kai-ken mah-dol-li-sek-si."},
        ],
    }
...
def generate_all_audio(phrases: list, output_dir: str):
    ...
    for i, phrase in enumerate(phrases):
        ...
        finnish_file = output_dir / f"finnish_{i}.mp3"
        ...
        fi_duration = get_audio_duration(str(finnish_file))

        # Add pause between English and Finnish
        pause_between = 0.5
        total_duration = en_duration + pause_between + fi_duration

        print(f"    ⏱️  Total: {total_duration:.2f}s (EN: {en_duration:.2f}s + pause: {pause_between}s + FI: {fi_duration:.2f}s)")
        ...
        if result.returncode != 0:
            ...
            with open(concat_file, "w", encoding="utf-8") as f:
                ...
                f.write(f"file '{finnish_file.as_posix()}'\n")
        ...
        audio_files.append({
            ...
            "finnish": str(finnish_file),
            "combined": str(combined_file),
            "duration": actual_duration,
            "en_duration": en_duration,
            "fi_duration": fi_duration
        })
...
def generate_complete_image(phrase_data: dict, category_english: str, output_path: str):
    ...
    finnish = phrase_data.get("finnish", "")
    ...
    # Finnish text
    finnish_y = english_y + total_height + 110  # Increased from 100
    finnish_lines = wrap_text(finnish, font_large, VIDEO_WIDTH - 140)
    total_height = len(finnish_lines) * 95  # Increased from 75

    draw.rectangle(
        [(60, finnish_y - 55), (VIDEO_WIDTH - 60, finnish_y + total_height + 15)],
        fill=(80, 30, 30, 220)
    )

    for i, line in enumerate(finnish_lines):
        y_pos = finnish_y + (i * 95)  # Increased spacing
        draw.text(
            (VIDEO_WIDTH // 2, y_pos),
            line,
            fill=(255, 255, 0),
    ...
    pronunciation_y = finnish_y + total_height + 90  # Increased from 80
...
def generate_reel(category_english: str = None):
    ...
    print(f"\n{'='*80}")
    print(f"Category: {category_english} ({CATEGORIES_FINNISH[category_english]})")
    print(f"{'='*80}\n")
    ...
    for i, phrase in enumerate(phrases, 1):
        print(f"  {i}. {phrase['english']} → {phrase['finnish']}")
    ...
    print("\n[3/4] Generating audio (English + Finnish with 500ms pause)...")
    ...
    print(f"  🏷️  Branding: Velocity Finnish")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("🇫🇮 VELOCITY FINNISH - FACEBOOK REELS AUTOMATION 🇫🇮")
    print("="*80)
    print("\n✨ IMPROVED FEATURES:")
    ...
    print("  ✓ Velocity Finnish branding at bottom")
    ...
    for i, cat in enumerate(CATEGORIES_ENGLISH, 1):
        print(f"   {i:2d}. {cat} ({CATEGORIES_FINNISH[cat]})")
    print(f"\n📅 DAILY CAPACITY:")
